...
import React, { useRef, useState, useEffect } from "react";
import Image from "next/image";
import { CheckCircle, XCircle, ImageIcon, Loader2, ArrowRight, Upload, Plus } from "lucide-react";
import { HeroItem, HeroItemFormData } from "@/types/parasole/home/home";
import { formatImageUrl, getFirstImage } from "@/hooks/parasole/home/use-home-item";
...
export function HomeItemForm({
  ...
  const [formData, setFormData] = useState<HeroItemFormData>({
    ...
    description: "",
    image: null,
    images: null, // New field for multiple images
    index: 0,
    status: "ACTIVE"
  });
  ...
  useEffect(() => {
    if (initialData) {
      ...
      // Set image preview from either images array or image field
      if (initialData.images && Array.isArray(initialData.images) && initialData.images.length > 0) {
        // Set the main preview to the first image
        setImagePreview(formatImageUrl(initialData.images[0]));
        
        // Set all image previews
        const previews = initialData.images.map(img => formatImageUrl(img));
        setImagePreviews(previews);
      } else if (initialData.image) {
        setImagePreview(formatImageUrl(initialData.image));
        setImagePreviews([formatImageUrl(initialData.image)]);
      } else {
        setImagePreview("");
        setImagePreviews([]);
      }
  ...
  const handleMultipleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (e.target.files && e.target.files.length > 0) {
      ...
      if (oversizedFiles.length > 0) {
        setErrors(prev => ({ ...prev, images: `${oversizedFiles.length} image(s) exceed 2MB limit` }));
        return;
      }
      ...
      if (nonImageFiles.length > 0) {
        setErrors(prev => ({ ...prev, images: `${nonImageFiles.length} file(s) are not images` }));
        return;
      }
      
      // Update form data with the first image for backward compatibility
      setFormData(prev => ({
        ...prev,
        image: filesArray[0],
        images: filesArray // Store all files in the images field
      }));
  ...
  const handleRemoveImage = (index: number) => {
    ...
    URL.revokeObjectURL(newImagePreviews[index]); // Clean up the URL object
    ...
    if (newImageFiles.length > 0) {
      setFormData(prev => ({
        ...prev,
        image: newImageFiles[0],
        images: newImageFiles
      }));
      setImagePreview(newImagePreviews[0]);
    } else {
      setFormData(prev => ({
        ...prev,
        image: null,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Image validation for new items (not required when editing an item that already has images)
    if (!initialData && !formData.image && (!formData.images || formData.images.length === 0)) {
      newErrors.image = "At least one image is required for new items";
    }
    ...
    const submissionData: HeroItemFormData = {
      ...formData,
        ...
        <div className="overflow-y-auto p-1">
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            <div className="space-y-6">
              ...
              <div className="bg-gray-50 p-6 rounded-xl border border-gray-100">
                <label className="block text-sm font-medium text-gray-700 mb-3">
                  Content Images {!initialData && <span className="text-red-500">*</span>}
                </label>
                ...
                <div className="flex flex-col space-y-4">
                  ...
                  <div 
                    ...
                    className={`w-full py-4 px-4 border-2 border-dashed ${errors.images ? 'border-red-300 bg-red-50' : 'border-blue-200 bg-blue-50'} rounded-lg cursor-pointer hover:bg-blue-100 transition-colors duration-200 flex items-center justify-center`}
                  >
                    <div className="text-center">
                      <Upload className="h-6 w-6 text-blue-500 mx-auto mb-2" />
                      <p className="text-sm font-medium text-blue-700">
                  ...
                  {errors.images && (
                    <p className="mt-2 text-sm text-red-600 flex items-center">
                      <XCircle className="h-4 w-4 mr-1" /> {errors.images}
                    </p>
                  )}
                  ...
                  {imagePreviews.length > 0 && (
                    <div className="mt-4">
                      <div className="text-sm font-medium text-gray-700 mb-2">Selected Images ({imagePreviews.length})</div>
                      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3">
                        {imagePreviews.map((preview, index) => (
                          <div key={index} className="relative group">
                            <div className="h-24 w-full rounded-lg overflow-hidden border border-gray-200 shadow-sm bg-white">
                              <img 
                                src={preview} 
                                alt={`Preview ${index + 1}`} 
                            ...
                            <button
                              ...
                              className="absolute -top-2 -right-2 bg-red-500 text-white p-1 rounded-full shadow-md opacity-0 group-hover:opacity-100 transition-opacity duration-200 hover:bg-red-600"
                            >
                        ...
                        <div 
                          onClick={triggerMultipleFileInput}
                          className="h-24 w-full rounded-lg border-2 border-dashed border-gray-300 flex items-center justify-center cursor-pointer hover:bg-gray-50 transition-colors duration-200"
                        >
                  ...
                  <p className="text-xs text-gray-500">
                    {initialData ? 
                      "Upload new images only if you want to change the current ones. The first image will be used as the main preview." : 
                      "Upload one or more images. The first image will be displayed as the main preview. Max size: 2MB per image."}
                  </p>
                </div>
                
                {/* Legacy single file input (hidden but kept for backward compatibility) */}
                <input
                  ref={fileInputRef}
                  id="image"
                  name="image"
