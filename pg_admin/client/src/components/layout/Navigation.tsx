...
const websiteConfigs: WebsiteConfigs = {
  ...
  paragon: {
    ...
    navItems: [
      ...
      { icon: Trophy, label: 'Milestones', path: '/admin/paragon/milestones', requiredPermission: 'paragon_group_view' },
      // Changed Business Activities from dropdown to single item
...
export function Navigation({ 
  ...
  const isSuperAdmin = (): boolean => {
    // Adjust this based on how you identify super admin in your system
    // This might be a specific role ID or a special flag
    return currentUser?.role === 'superadmin' || currentUser?.roleId === 1;
  }
                  ...
                  {isSidebarOpen && (
                    <div className="mt-1 space-y-1">
                      {websiteConfigs[selectedWebsite.slug].navItems
                        ...
                        .map((item, index) => {
                          // Check if item has subitems and it's NOT the Business Activities item (dropdown)
                          if (item.subItems && item.subItems.length > 0 && item.label !== 'Business Activities') {
                            return (
                              <NavDropdown
                                key={index}
                                icon={item.icon}
