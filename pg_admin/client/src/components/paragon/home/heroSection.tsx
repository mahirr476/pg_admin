...
const HeroSection: React.FC = () => {
  ...
  const [images, setImages] = useState<File[]>([]); // State for image files
  const [imageUrls, setImageUrls] = useState<string[]>([]); // For preview
  const [existingImages, setExistingImages] = useState<string[]>([]); // For existing images in edit mode
  const [isEditing, setIsEditing] = useState<boolean>(false);
  const [editId, setEditId] = useState<number | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent): Promise<void> => {
    ...
    try {
      ...
      // When editing, include existing images that weren't removed
      ...
      // Add new images to form data if there are any
      if (images.length > 0) {
        images.forEach(image => {
          formData.append('images', image);
        });
  ...
  const updateStatus = async (id: number, newStatus: string): Promise<void> => {
    try {
      ...
      // Include existing images when updating status
      if (heroToUpdate.images && heroToUpdate.images.length > 0) {
        heroToUpdate.images.forEach(image => {
          formData.append('existingImages', image);
        });
      }
  ...
  const handleEdit = (hero: HeroData): void => {
    ...
    // If the hero has images from the server, store them for existing images
    if (hero.images && hero.images.length > 0) {
  ...
  function renderTable(): JSX.Element {
    return (
      <div className="overflow-x-auto">
        <div className="overflow-auto rounded-xl border border-gray-200 shadow-lg">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gradient-to-r from-gray-50 to-gray-100">
              <tr>
                <th scope="col" className="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                      ...
                      <div className="flex items-center space-x-1">
                        {hero.images && hero.images.length > 0 ? (
                          <>
                            ...
                            {hero.images.slice(0, 3).map((image, imgIdx) => (
                              <div 
                                key={imgIdx} 
                                className="h-12 w-12 rounded-lg overflow-hidden bg-gray-100 border border-gray-200 flex-shrink-0"
                              >
                                <img 
                                  src={getFullImageUrl(image)} 
                                  alt={`Hero image ${imgIdx + 1}`}
                                  className="h-full w-full object-cover"
                                />
                              </div>
                            ))}
                            {/* Show additional count if more than 3 images */}
                            {hero.images.length > 3 && (
                              <div className="h-12 w-12 rounded-lg bg-indigo-100 border border-indigo-200 flex items-center justify-center flex-shrink-0">
