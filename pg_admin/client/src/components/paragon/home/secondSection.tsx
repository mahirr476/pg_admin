...
const SecondSection: React.FC = () => {
  ...
  React.useEffect(() => {
    const savedMain = localStorage.getItem('secondMainSection');
    const savedNumbered = localStorage.getItem('secondNumberedSections');
    ...
    if (savedNumbered) {
      ...
      const initialSections = Array(6).fill(null).map((_, index) => ({
      ...
      localStorage.setItem('secondNumberedSections', JSON.stringify(initialSections));
    }
  }, []);
  ...
  React.useEffect(() => {
    if (showData) {
      localStorage.setItem('secondMainSection', JSON.stringify(mainSection));
      localStorage.setItem('secondNumberedSections', JSON.stringify(numberedSections));
    }
  ...
  const handleTempNumberedSectionChange = (index: number, e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    <div className="space-y-8">
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogTrigger asChild>
          <Button onClick={handleDialogOpen}>Impact Section</Button>
        </DialogTrigger>
      ...
      {showData && (
        <Card className="p-6">
          ...
          {numberedSections.some(section => section.number || section.title || section.description) && (
            <div className="space-y-4">
                ...
                <Table>
                  ...
                  <TableBody>
                    {numberedSections.map((section) => (
                      <TableRow 
                        key={section.id}
                        className={section.number || section.title || section.description ? 'bg-white hover:bg-slate-50' : 'hidden'}
                      >
