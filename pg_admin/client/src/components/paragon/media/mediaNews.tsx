...
import { Edit, Trash2, ToggleLeft, ToggleRight, Plus, X, Image, Link, Calendar, Tag } from 'lucide-react';
...
const MediaNews = () => {
  ...
  const fetchNewsData = async () => {
    try {
      setLoading(true);
      const response = await api.get('/group/media/news');
      if (response.data.success) {
        setNewsData(Array.isArray(response.data.data) ? response.data.data : [response.data.data].filter(Boolean));
      } else {
    ...
    try {
      ...
      if (isEditing) {
        ...
        const response = await axios.put(
          `http://localhost:7000/api/v1/group/media/news/${formData.id}`, 
          formDataObj, 
          { headers }
        );
        ...
        const response = await axios.post(
          'http://localhost:7000/api/v1/group/media/news', 
          formDataObj, 
          { headers }
        );
  ...
  const handleEdit = (item) => {
    ...
    if (item.date && !item.date.match(/^\d{4}-\d{2}-\d{2}$/)) {
      try {
        // Try to parse the date string and format for the date input
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this item?')) {
      try {
        const response = await api.delete(`/group/media/news/${id}`);
        if (response.data.success) {
          fetchNewsData(); // Refresh data
        } else {
  ...
  const toggleStatus = async (id, currentStatus) => {
    try {
      ...
      const response = await api.put(`/group/media/news/${id}`, { 
        status: newStatus 
      });
  ...
  return (
    <div className="container mx-auto">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-semibold">Media News Management</h2>
        <button 
      ...
      <div className="bg-white shadow-md rounded overflow-hidden">
        {loading ? (
          ...
          <div className="text-center p-4">No news items found</div>
        ) : (
              ...
              <tbody>
                {newsData.map((item) => (
                  <tr key={item.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4">{truncateText(item.title, 30)}</td>
                    <td className="py-3 px-4">
                      <div className="line-clamp-2">{truncateText(item.description, 50)}</div>
                    </td>
                    <td className="py-3 px-4">
                      {item.image && (
                    ...
                    <td className="py-3 px-4">
                      <div className="truncate max-w-[150px]">{item.link}</div>
                    </td>
                    <td className="py-3 px-4">
          ...
          <div className="bg-white rounded-lg shadow-lg w-full max-w-xl mx-4">
            ...
            <form onSubmit={handleSubmit} className="p-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2 flex justify-end mt-6 space-x-2">
                  <button
                    type="button"
                    onClick={handleCloseModal}
