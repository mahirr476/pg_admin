                                            ...
                                            <TableRow key={business.id}>
                                                ...
                                                <TableCell>
                                                    <div className="flex items-center space-x-2">
                                                        <div className={`px-3 py-1 rounded-full text-xs font-medium ${business.status === "ACTIVE"
                                                                ? "bg-green-100 text-green-800 border border-green-300"
                                                                : "bg-red-100 text-red-800 border border-red-300"
                                                            }`}>
                                                            <div className="flex items-center">
                                                                <div className={`w-2 h-2 mr-1.5 rounded-full ${business.status === "ACTIVE" ? "bg-green-600" : "bg-red-600"
                                                                    }`}></div>
                                                                {business.status}
                                                            </div>
                                                        </div>
                                                            ...
                                                            onClick={async () => {
                                                                ...
                                                                try {
                                                                    ...
                                                                    for (const option of endpointOptions) {
                                                                        console.log(`Trying endpoint: ${option.url} with method: ${option.method}`);
                                                                        ...
                                                                        try {
                                                                            ...
                                                                            console.log(`${option.url} response:`, responseText);

                                                                            if (!response.ok) {
                                                                                console.log(`Endpoint ${option.url} failed with status ${response.status}`);
                                                                            ...
                                                                            try {
                                                                                ...
                                                                                if (data.success) {
                                                                                    ...
                                                                                    console.log(`Endpoint ${option.url} returned success: false`);
                                                                                }
                                                                            } catch (e) {
                                                                                console.log(`Failed to parse JSON from ${option.url}`);
                                                                            }
                                                                        } catch (err) {
                                                                            console.log(`Error with endpoint ${option.url}:`, err);
                                                                        }
                                                                    }

                                                                    if (!succeeded) {
                                                                        // If all options failed, show an error
                                                                        throw new Error("All status update attempts failed. Please check API documentation.");
                                                                    }
                                                                } catch (error: any) {
                                                                    console.error("Error updating status:", error);
                                                                    showNotification("Error", error.message || "Failed to update status", true);
