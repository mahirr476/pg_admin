...
export const useHomeDetailItem = () => {
  ...
  const handleAuthError = (error: any, response?: Response) => {
    ...
    if (error.message === 'Failed to fetch') {
      setError("Network error. Please check your connection and ensure the server is running on http://localhost:7000");
      return;
    }
  ...
  const toggleHeroDetailStatus = useCallback(async (id: number, currentStatus: string): Promise<boolean> => {
    ...
    try {
      ...
      // Try a simple PUT request with just the status field
      const updateData = { status: newStatus };
      ...
      if (!response.ok) {
        ...
        setHeroDetails(heroDetails.map(detail => 
          detail.id === id ? { ...detail, status: currentStatus } : detail
        ));
      ...
      // Revert the UI change on network error
      setHeroDetails(heroDetails.map(detail => 
        detail.id === id ? { ...detail, status: currentStatus } : detail
      ));
