...
import { 
  ...
  getMediaNews, 
  getVideoGallery 
} from "../../../services/group/client/media.service";
import { 
...
export const MediaController = {
    getMedia: async (req: Request, res: Response): Promise<void> => {
        try {
            // Fetch all media data concurrently
            const [mediaContent, videoGallery, mediaNews, mediaInquiries] = await Promise.all([
                getMediaContent(),
                getVideoGallery(),
                getMediaNews(),
                getMediaInquiries()
            ...
            res.status(500).json({
                success: false,
                message: error instanceof Error ? error.message : 'Failed to fetch Media data'
            });
        }
    },
