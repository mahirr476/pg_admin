...
export const CompaniesController = {
    getCompanies: async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            res.status(500).json({
                success: false,
                message: error instanceof Error ? error.message : 'Failed to fetch Companies data'
            });
        }
    },
    ...
    getCompaniesBySlug: async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            res.status(500).json({
                success: false,
                message: error instanceof Error ? error.message : "Failed to fetch Companies details."
            });
        }
    }
};
