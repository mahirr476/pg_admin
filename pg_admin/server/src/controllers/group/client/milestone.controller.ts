...
import { 
  getActiveMilestone, 
  getMilestoneContent 
} from "../../../services/group/client/milestone.service";
...
export const MilestoneController = {
    getMilestoneContent: async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            res.status(500).json({
                success: false,
                message: error instanceof Error ? error.message : 'Failed to fetch Milestone Content'
            });
        }
    },
    ...
    getAllData: async (req: Request, res: Response): Promise<void> => {
        try {
            // Fetch both milestone data sets concurrently
            const [milestoneContent, milestone] = await Promise.all([
                getMilestoneContent(),
                getActiveMilestone()
            ]);
            ...
            res.status(500).json({
                success: false,
                message: error instanceof Error ? error.message : 'Failed to fetch Milestone page data'
            });
        }
    },
};
