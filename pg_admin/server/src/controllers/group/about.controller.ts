...
// // const UPLOAD_DIR = path.join(__dirname, '..', '..', 'public', 'uploads', 'group', 'about');
// // const UPLOAD_DIR = "D:/Devlopment/pg_admin/pg_admin/server/public/uploads/group/about";
// const UPLOAD_DIR = "/app/public/uploads/group/about";
...
import { UPLOAD_PATHS, uploadAboutImage } from "../../middleware/upload.middleware";
import fs from 'fs';
import path from 'path';
...
export const AboutController = {
    ...
    upsert: async (req: Request, res: Response): Promise<void> => {
        uploadAboutImage(req, res, async (err: any) => {
            if (err) {
            ...
            try {
                ...
                // Validate required fields
                if (!data.title || !data.description || !data.mission || !data.vision || !data.about) {
                    throw new Error("Title, description, mission, vision, and about are required fields.");
                }

                // Get existing about data to check for existing image
                const existingAbout = await getAbout();
                
                // Handle image processing - use explicit null check
                imagePath = existingAbout && existingAbout.image ? existingAbout.image : null;
                
                // If a new file was uploaded, update the image path and delete old file if it exists
                if (req.file) {
                    // Store the relative path for the database
                    imagePath = `${UPLOAD_PATHS.ABOUT_GROUP_IMAGES}/${req.file.filename}`;
                    
                    // Delete old image if it exists and is different from default
                    if (existingAbout?.image && existingAbout.image !== 'default-image.jpg') {
                        try {
                            // Construct full path to old image
                            const oldImageFullPath = path.join(process.cwd(), existingAbout.image);
                ...
                res.status(existingAbout ? 200 : 201).json({
                    success: true,
                    message: existingAbout ? "About information updated successfully." : "About information created successfully.",
                    data: formattedAbout,
                });
                ...
                // Delete uploaded image if an error occurs and it's a new upload
