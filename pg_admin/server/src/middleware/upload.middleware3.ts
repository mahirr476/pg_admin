...
// ... [keep existing middleware functions] ...

// New flexible upload middleware for Hero images
export const uploadHeroImages = (req: any, res: any, next: any) => {
  // Determine if it's a single upload or multiple upload request
  ...
  const storage = multer.diskStorage({
    destination: function (req, file, cb) {
      // Ensure the directory exists
      ensureUploadDirExists(UPLOAD_PATHS.HERO_IMAGES);
      cb(null, UPLOAD_PATHS.HERO_IMAGES);
    },
    filename: function (req, file, cb) {
      // Create a more secure filename while preserving original extension
      const uniqueSuffix = `${Date.now()}-${Math.round(Math.random() * 1e9)}`;
  ...
  const fileFilter = (req: any, file: any, cb: any) => {
    // Accept only images
    if (file.mimetype.startsWith('hero/')) {
      cb(null, true);
    } else {
      cb(new Error('Only image files are allowed'));
    }
  ...
  // Use either single or array based on the request
