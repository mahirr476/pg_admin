...
export const getAllMediaContacts = async (page = 1, limit = 10) => {
  try {
    // Calculate the number of records to skip
    const skip = (page - 1) * limit;
    
    // Get the total count of media contacts for pagination metadata
    const totalCount = await group.mediaContact.count();
    
    // Calculate total pages
    const totalPages = Math.ceil(totalCount / limit);
    
    // Get paginated media contacts
    const contacts = await group.mediaContact.findMany({
      orderBy: {
        createdAt: 'desc'
      },
      skip,
    ...
    // Return both the contacts and pagination metadata
    return {
      contacts,
      pagination: {
        total: totalCount,
