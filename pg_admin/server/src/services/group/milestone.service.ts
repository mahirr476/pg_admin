...
export const createMilestone = async (data: CreateMilestoneInput) => {
  // Check if orderIndex is already in use
  const existingMilestone = await group.milestone.findFirst({
    where: { orderIndex: data.orderIndex },
  });

  if (existingMilestone) {
    throw new Error(`A milestone with order index ${data.orderIndex} already exists. Please use a unique order index.`);
  }
  ...
  return await group.milestone.create({
    data: {
      ...
      image: data.image || [], // Default to empty array if no images are provided
...
export const getMilestoneById = async (id: number) => {
  return await group.milestone.findUnique({
      where: { id }
  });
};

export const updateMilestone = async (id: number, data: UpdateMilestoneInput) => {
  // Check if orderIndex is already in use by another milestone
